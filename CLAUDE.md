...
- `docs/telemetry-pull-layer.md` — Telemetry pull layer: on-machine transcript parsers that attribute tokens to every span; retained as the Langfuse backfill source (the Streamlit dashboard + DuckDB store were removed in #90; `ccusage` + the pull-cost layer were retired in #91 — Langfuse computes cost from token usage)
