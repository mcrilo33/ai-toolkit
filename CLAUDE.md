...
- `docs/telemetry-pull-layer.md` — Telemetry pull layer: the land-time view builder (`langfuse_spoke_tree.py`) that assembles the `spoketree-`/`spokecycle-` views from on-machine transcript parsing; no backfill/healing path since #140 (the Streamlit dashboard + DuckDB store were removed in #90; `ccusage` + the pull-cost layer were retired in #91 — Langfuse computes cost from token usage)
