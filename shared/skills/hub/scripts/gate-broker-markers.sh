...
# park-sig-<issue> — the park SIGNATURE the live park-onset epoch belongs to (issue #283).
...
# ceiling measured against it. Pairing the onset with the signature of the park it was stamped for
# closes that: a changed signature is a new episode however busy the spoke is.
...
# first when this is a new episode. New means either the pending park's signature differs from the
# one the live onset was stamped for, or the onset is gone while the signature record survives
# (slot_state's clear_park_onset_epoch drops the epoch on a not-parked tick and knows nothing about
# this record, so an identical dialog re-raised later must not inherit the dead episode's age).
# An empty signature — _broker_park_signature fail-opens to empty on an unextractable park, e.g. a
# gate tag whose plan artifact is unreadable (markers.sh: `gate:` -> empty) — records NOTHING and
# leaves the onset exactly as stamp-once left it: never claim an episode we cannot substantiate.
# Called by the watchdog (park-onset's only reader) on each waiting tick; best-effort throughout.
note_park_episode() {
  local wt="$1" issue="$2" sig prev onset
  sig="$(_broker_park_signature "$wt" "$issue" 2>/dev/null)"
  [ -n "$sig" ] || { read_park_onset_epoch "$issue"; return 0; }
  prev="$(read_park_sig "$issue")"
  onset="$(read_park_onset_epoch "$issue")"
  case "$onset" in '' | *[!0-9]*) onset="" ;; esac
  if [ "$prev" != "$sig" ] || [ -z "$onset" ]; then
    stamp_park_onset_epoch "$issue"
    mkdir -p "$(_afk_state_dir)" 2>/dev/null || true
    printf '%s\n' "$sig" > "$(_park_sig_file "$issue")" 2>/dev/null || true
  fi
  read_park_onset_epoch "$issue"
}
