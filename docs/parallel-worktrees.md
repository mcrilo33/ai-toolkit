...
rolls `main` back (`git reset --keep`) with nothing pushed. Every push the worktree
scripts perform carries SSH keepalive options (`ServerAliveInterval`, issue #119) so
the multi-minute in-push gate can't stale the connection; if the gate ran green and
the transfer still died, the land retries once with `TEST_SELECT_SKIP=1` — loudly — and
a failed gate never retries. On success it calls
