...
The adaptive AFK dispatcher (issue #41, Phase 1 of epic #40) drains a queue of
`afk`-labelled issues while away with a self-tuning concurrency cap. This file
...
HUB_AFK = (
    Path(__file__).resolve().parents[2] / "shared" / "skills" / "hub" / "scripts" / "hub-afk.sh"
)
...
# --- afk_target: the clamp formula -----------------------------------------


@pytest.mark.parametrize(
    "tasks_left,time_left,expected",
    [
        (5, 480, 1),  # queue fits a full window -> sequential
        (1, 480, 1),  # single task -> sequential
        (3, 90, 3),  # ceil(270/90) = 3, exactly at the cap
        (5, 150, 3),  # window burning down -> ramps to the cap
        (20, 480, 3),  # ceil(3.75) = 4, clamped to MAX=3
        (0, 480, 1),  # empty -> clamped up to the floor of 1
    ],
)
def test_afk_target_worked_cases(tasks_left: int, time_left: int, expected: int) -> None:
    result = _call(f"afk_target {tasks_left} {time_left}")

    assert result.returncode == 0, result.stderr
    assert result.stdout.strip() == str(expected)


def test_afk_target_honors_task_minutes_knob() -> None:
    # T_task=60: ceil(10*60/120) = 5, clamped to the default MAX of 3.
    result = _call("afk_target 10 120", env={"AFK_TASK_MINUTES": "60"})

    assert result.stdout.strip() == "3"


def test_afk_target_honors_max_concurrency_knob() -> None:
    # Lift the cap to 5: ceil(20*90/480) = 4 is now under the cap.
    result = _call("afk_target 20 480", env={"AFK_MAX_CONCURRENCY": "5"})

    assert result.stdout.strip() == "4"


def test_afk_target_guards_nonpositive_time_left() -> None:
    # time_left <= 0 (the window is over) must not divide-by-zero; it pins to MAX.
    result = _call("afk_target 5 0")

    assert result.returncode == 0, result.stderr
    assert result.stdout.strip() == "3"


# --- minutes_until: the AFK_UNTIL absolute-end fallback clock -----------------
...
# --- The away window: duration (AFK_FOR), fallback, pinned deadline ----------
...
def test_parse_duration_accepts_hours_minutes_and_bare(spec: str, minutes: int) -> None:
    result = _call(f"parse_duration {spec}")

    assert result.returncode == 0, result.stderr
    assert result.stdout.strip() == str(minutes)


def test_parse_duration_rejects_malformed_spec() -> None:
    result = _call("parse_duration 8x")

    assert result.returncode != 0
    assert "malformed" in result.stderr.lower()


def test_afk_window_uses_duration_when_afk_for_set() -> None:
    # AFK_FOR wins over AFK_UNTIL: the window is the duration, not the clock time.
    now = _epoch(2026, 6, 15, 23, 0)

    result = _call(f"afk_window_minutes {now}", env={"AFK_FOR": "6h", "AFK_UNTIL": "07:00"})

    assert result.stdout.strip() == "360"


def test_afk_window_falls_back_to_afk_until_without_duration() -> None:
    # No AFK_FOR -> the absolute-end fallback: 23:00 -> next 07:00 = 480 minutes.
    now = _epoch(2026, 6, 15, 23, 0)

    result = _call(f"afk_window_minutes {now}", env={"AFK_FOR": "", "AFK_UNTIL": "07:00"})

    assert result.stdout.strip() == "480"


def test_deadline_is_pinned_and_resumed_across_restarts(tmp_path: Path) -> None:
    # The deadline is computed ONCE (now + AFK_FOR) and persisted, so a later
    # invocation with a fresh "now" resumes the SAME deadline instead of sliding
    # the window forward every restart.
    state = tmp_path / "afk-state"
    start = _epoch(2026, 6, 15, 23, 0)
    env = {"AFK_FOR": "8h", "AFK_STATE_DIR": str(state)}

    first = _call(f"afk_deadline_epoch {start}", env=env)
    later = _call(f"afk_deadline_epoch {start + 3600}", env=env)  # relaunched 1h later

    assert first.returncode == 0, first.stderr
    assert int(first.stdout.strip()) == start + 8 * 3600
    assert later.stdout.strip() == first.stdout.strip()  # resumed, not re-anchored


def test_minutes_left_counts_down_against_the_pinned_deadline(tmp_path: Path) -> None:
    state = tmp_path / "afk-state"
    start = _epoch(2026, 6, 15, 23, 0)
    env = {"AFK_FOR": "8h", "AFK_STATE_DIR": str(state)}

    at_start = _call(f"minutes_left {start}", env=env)
    one_hour_in = _call(f"minutes_left {start + 3600}", env=env)

    assert at_start.stdout.strip() == "480"
    assert one_hour_in.stdout.strip() == "420"


# --- Dispatch tick (--once): queue, idempotent skip, cutoff ------------------
# These run the script end-to-end for one tick from a hub checkout, with `gh`
# (the afk queue) and worktree-new.sh (WT_NEW, the dispatcher) stubbed so no
...
@pytest.fixture()
def afk_hub(tmp_path: Path) -> Path:
    ...
    via the ``gh`` stub's ``AFK_QUEUE`` env var.
    """
...
def _run_once(
    ...
    *,
    ...
    """Run ``hub-afk.sh`` from the hub with gh + WT_NEW stubbed.
    ...
    Args:
        ...
        now: Injected current time (epoch seconds) -> AFK_NOW.
    ...
    # Unset AFK_BODIES_DIR (no bodies) -> every issue reads as empty -> no
    ...
    gh.write_text(
        ...
        '  for n in $AFK_QUEUE; do echo "$n"; done\n'
        "  exit 0\n"
        "fi\n"
        'if [ "$1" = "issue" ] && [ "$2" = "view" ]; then\n'
        '  f="$AFK_BODIES_DIR/$3"\n'
        '  [ -n "$AFK_BODIES_DIR" ] && [ -f "$f" ] && cat "$f"\n'
        "  exit 0\n"
        "fi\n"
        "exit 1\n"
    )
...
def test_dispatch_up_to_target_from_queue(afk_hub: Path, tmp_path: Path) -> None:
    # 5 queued, 150 min left -> target ceil(5*90/150)=3; no in-flight -> 3 spawn.
    now = _epoch(2026, 6, 15, 4, 30)  # 07:00 - 150m

    proc, lines = _run_once(afk_hub, tmp_path, queue="101 102 103 104 105", now=now)
...
def test_inflight_issue_is_not_redispatched(afk_hub: Path, tmp_path: Path) -> None:
    ...
    _add_inflight(afk_hub, tmp_path, 101)
    now = _epoch(2026, 6, 15, 4, 30)

    proc, lines = _run_once(afk_hub, tmp_path, queue="101", now=now)

    assert proc.returncode == 0, proc.stderr
    assert lines == []


def test_launch_cutoff_blocks_all_dispatch(afk_hub: Path, tmp_path: Path) -> None:
    # 60 min left (< T_task=90) -> strict cutoff: nothing is started.
    now = _epoch(2026, 6, 15, 6, 0)  # 07:00 - 60m

    proc, lines = _run_once(afk_hub, tmp_path, queue="101 102", now=now)

    assert proc.returncode == 0, proc.stderr
    assert lines == []


def test_dispatch_passes_type_feature_and_prompt(afk_hub: Path, tmp_path: Path) -> None:
    now = _epoch(2026, 6, 15, 4, 30)

    _, lines = _run_once(afk_hub, tmp_path, queue="101 102 103 104 105", now=now)

    assert lines, "expected at least one dispatch"
    assert all("type=yes" in ln and "prompt=yes" in ln for ln in lines)


# --- The AFK kickoff: agent-review + park-don't-guess (issue #40 ST4) --------
# The behavioral inversion of the daytime kickoff. Daytime parks at the PLAN gate
# for a human (gate/N); while away the judgment gates route to an independent
...
def test_afk_kickoff_is_park_dont_guess() -> None:
    proc, flat = _kickoff_flat()

    assert proc.returncode == 0, proc.stderr
    assert "unattended" in flat, "the AFK kickoff must state it runs unattended"
    assert "park" in flat
...
def test_afk_kickoff_routes_judgment_gates_to_adversarial_review() -> None:
    proc, flat = _kickoff_flat()
...
def test_afk_kickoff_emits_terminal_markers_via_script() -> None:
    proc, flat = _kickoff_flat()

    assert proc.returncode == 0, proc.stderr
    for marker in ("ready/", "accept/", "blocked/"):
        assert marker in flat, f"the AFK kickoff must name the {marker} terminal marker"
    ...
    # non-existent spoke-ready.sh --ready flag — a literal-following afk spoke must
...
def test_kickoff_is_not_plan_mode() -> None:
    ...
    afk spoke. Lock it here so it can't be reintroduced.
    """
...
# branch tip) or idle (its newest transcript is older than AFK_IDLE_MINUTES);
# a busy spoke keeps occupying one. The supervisor backfills freed slots from
# the still-pending queue. With a full window the target is 1, so a backfill can
...
def _seed_transcript(projects_dir: Path, wt_path: Path, *, mtime: int) -> Path:
    ...
    realpaths, e.g. /private/var on macOS), matching hub-afk's lookup.
    """
...
def test_terminal_marker_frees_slot_for_backfill(afk_hub: Path, tmp_path: Path, kind: str) -> None:
    # 101 in flight + <kind>/101 at its tip -> terminal -> doesn't occupy a slot,
    # so the pending 102 backfills even though the full-window target is 1. This is
    # the accept/blocked counterpart of test_done_spoke_frees_slot_for_backfill.
    _add_inflight(afk_hub, tmp_path, 101)
    _seed_terminal_tag(afk_hub, kind, 101)
...
def test_gate_marker_does_not_free_slot(afk_hub: Path, tmp_path: Path) -> None:
    # gate/101 is the NON-terminal PLAN park: a gate-parked spoke is awaiting
    # review and still owns its slot, so under a full-window target of 1 the
    # pending 102 is NOT backfilled. (Contrast the accept/blocked case above.)
    _add_inflight(afk_hub, tmp_path, 101)
    _git(afk_hub, "tag", "gate/101", "feature/101-wip")
...
def test_done_spoke_frees_slot_for_backfill(afk_hub: Path, tmp_path: Path) -> None:
    # 101 in flight + ready/101 at its tip -> done -> doesn't occupy a slot, so
    # the pending 102 is backfilled even though the full-window target is 1.
    _add_inflight(afk_hub, tmp_path, 101)
    _seed_ready_tag(afk_hub, 101)
    now = _epoch(2026, 6, 15, 1, 0)  # ~6h to 07:00 -> target 1
...
def test_idle_spoke_is_reaped_then_frees_slot(afk_hub: Path, tmp_path: Path) -> None:
    # 101 in flight but its transcript is 20 min stale (> AFK_IDLE_MINUTES=15)
    # with NO terminal/gate marker -> hung -> the supervisor REAPS it: emits
    # blocked/101 on its behalf (so the return report shows a THINK row instead
    ...
    wt = _add_inflight(afk_hub, tmp_path, 101)
    projects = tmp_path / "projects"
    now = _epoch(2026, 6, 15, 1, 0)
    _seed_transcript(projects, wt, mtime=now - 20 * 60)

    proc, lines = _run_once(afk_hub, tmp_path, queue="101 102", now=now, projects_dir=projects)

    assert proc.returncode == 0, proc.stderr
    assert _git(afk_hub, "tag", "-l", "blocked/101").strip() == "blocked/101", (
        "a hung idle spoke must be reaped with a blocked/N marker"
    )
    assert [ln.split()[1] for ln in lines] == ["issue=102"]


def test_busy_spoke_occupies_slot_and_blocks_backfill(afk_hub: Path, tmp_path: Path) -> None:
    # 101 in flight, transcript active 1 min ago -> busy -> occupies the only
    # slot (full-window target 1), so pending 102 is NOT backfilled this tick.
    wt = _add_inflight(afk_hub, tmp_path, 101)
    projects = tmp_path / "projects"
    now = _epoch(2026, 6, 15, 1, 0)
...
# hub-afk.sh had NO per-spoke runtime kill: launch_cutoff only gates NEW
...
def _epoch_file(tmp_path: Path, issue: int) -> Path:
    """The persisted dispatch-epoch file the supervisor stamps (AFK_STATE_DIR)."""
    return tmp_path / "afk-state" / f"dispatch-{issue}.epoch"
...
def test_spoke_over_ceiling(age_min: int, over: bool) -> None:
    ...
    # longer than AFK_SPOKE_MAX_MINUTES (default 180 = 2*T_task). Strict ">".
...
def test_dispatch_stamps_a_persisted_epoch(afk_hub: Path, tmp_path: Path) -> None:
...
def test_over_ceiling_spoke_is_reaped(afk_hub: Path, tmp_path: Path) -> None:
...
def test_gate_parked_idle_spoke_is_not_reaped(afk_hub: Path, tmp_path: Path) -> None:
    ...
    assert proc.returncode == 0, proc.stderr
    assert not _git(afk_hub, "tag", "-l", "blocked/101").strip(), (
...
def test_serial_after_waits_for_unlanded_predecessor(afk_hub: Path, tmp_path: Path) -> None:
...
def test_serial_after_releases_once_predecessor_landed(afk_hub: Path, tmp_path: Path) -> None:
...
def test_merge_child_is_never_dispatched(afk_hub: Path, tmp_path: Path) -> None:
...
# window is over or there is nothing left to supervise. timeout in _run_once is
# the safety net that turns a hang into a test failure.


def test_loop_exits_when_queue_empty_and_nothing_in_flight(afk_hub: Path, tmp_path: Path) -> None:
    now = _epoch(2026, 6, 15, 1, 0)

    proc, lines = _run_once(afk_hub, tmp_path, queue="", now=now, once=False)

    assert proc.returncode == 0, proc.stderr
    assert lines == []


def test_loop_exits_at_launch_cutoff(afk_hub: Path, tmp_path: Path) -> None:
    # Past the cutoff there is nothing left to launch -> the loop ends rather
    # than spinning forever waiting for a window that will not open during this window.
    now = _epoch(2026, 6, 15, 6, 30)  # 30 min to 07:00 (< T_task)

    proc, lines = _run_once(afk_hub, tmp_path, queue="101 102", now=now, once=False)
