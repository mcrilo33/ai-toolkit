...
def _run_guard_child(tmp_path: Path) -> subprocess.CompletedProcess[str]:
    ...
    (childdir / "test_inner.py").write_text(
        ...
        "\n"
        "def _cfg(numprocesses, file_or_dir=None):\n"
        "    return types.SimpleNamespace(option=types.SimpleNamespace(\n"
        "        numprocesses=numprocesses, file_or_dir=file_or_dir or []))\n"
        "\n"
        "def test_guard_fires_on_bare_whole_suite_xdist():\n"
        "    # Bare `-n auto` (no explicit file selection) collecting serial items -> fire.\n"
        ...
        "\n"
        "def test_guard_quiet_on_explicit_selection_under_xdist():\n"
        "    # The SELECTED leg names mapped files explicitly (file_or_dir non-empty);\n"
        "    # even under xdist that is the caller's deliberate choice, not the bare\n"
        "    # whole-suite run the guard exists to catch (#328 review blocker).\n"
        "    cfg = _cfg('auto', file_or_dir=['tests/unit/test_tripwire.py'])\n"
        "    conftest.pytest_collection_modifyitems(cfg, [_Item(True)])\n"
    )
