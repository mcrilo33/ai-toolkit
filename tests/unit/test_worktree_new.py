...
def test_agent_launch_appends_budget_args_when_set(hub: Path, tmp_path: Path) -> None:
    # A best-effort in-process budget cap for unattended AFK spokes: hub-afk
    # sets WT_AGENT_BUDGET_ARGS from AFK_MAX_BUDGET_USD, appended after the model
    # (before any seeded prompt). Attended spokes leave it unset → launch unchanged.
