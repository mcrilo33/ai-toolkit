"""Per-spoke cost for the AFK return report.

The return report needs each spoke's cost without re-deriving it. ``cost_for_issue``
...
The ``--cost-for`` CLI is what ``hub-return.sh`` shells out to per row; it degrades
...
def _print_cost(issue: int, events: Path | None, projects: Path | None) -> None:
    ...
    try:
        ...
        # Degrade to blank — a missing cost must never break the return report.
        return


def _main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="afk return-report cost lookup")
