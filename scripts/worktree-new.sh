...
# Best-effort in-process budget cap for unattended (AFK) spokes. hub-afk sets
# WT_AGENT_BUDGET_ARGS (e.g. "--max-budget-usd 5") from AFK_MAX_BUDGET_USD; it is
# a pre-formed multi-arg string appended verbatim (NOT %q-quoted), so leave it
# unset for ordinary attended spokes to keep the launch unchanged. The supervisor-side
