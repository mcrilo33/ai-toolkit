...
sync_workflow_scripts() {
    ...
    local name src
    for name in worktree-new.sh worktree-land.sh worktree-done.sh worktree-lib.sh worktree-quick.sh spoke-push.sh spoke-ready.sh spoke-relaunch.sh gate-sweep.sh travel-local.sh telemetry-ingest-spoke.sh hub-status.sh hub-ready-watch.sh hub-notify.sh hub-otel-watch.sh hub-afk.sh hub-agent.sh gate-broker.sh gate-broker-markers.sh gate-broker-detect.sh gate-broker-classify.sh hub-inject.sh hub-watchdog.sh batch-plan.sh telemetry.sh base-branch.sh enabled.sh; do
        case "$name" in
            hub-status.sh|hub-ready-watch.sh|hub-notify.sh|hub-otel-watch.sh|hub-afk.sh|hub-agent.sh|gate-broker.sh|gate-broker-markers.sh|gate-broker-detect.sh|gate-broker-classify.sh|hub-inject.sh|hub-watchdog.sh|batch-plan.sh) src="$SHARED_DIR/skills/hub/scripts/$name" ;;
