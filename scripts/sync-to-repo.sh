...
# scripts/, hub-status.sh, hub-ready-watch.sh and hub-afk.sh under the hub
...
sync_workflow_scripts() {
    ...
    local name src
    for name in worktree-new.sh worktree-land.sh worktree-done.sh worktree-lib.sh spoke-push.sh spoke-ready.sh hub-status.sh hub-ready-watch.sh hub-afk.sh hub-scout.sh hub-return.sh telemetry.sh; do
        case "$name" in
            hub-status.sh|hub-ready-watch.sh|hub-afk.sh|hub-scout.sh|hub-return.sh) src="$SHARED_DIR/skills/hub/scripts/$name" ;;
