...
# ensure-test-venv.sh ships so worktree-new.sh can provision the pre-push gate's
# testmon/xdist deps into each spawn's .venv (issue #342); toolkit-root script, so
# it too takes the loop's default source case.
# worktree-otel-lib.sh / worktree-gh-lib.sh are the two modules issue #353 split out
# of worktree-lib.sh; the thin entry sources them as co-located siblings, so both
# MUST ship next to worktree-lib.sh (toolkit-root default case) or a synced target's
# entry sources a missing sibling and warns loudly (OTel preflight / gh labels dead).
